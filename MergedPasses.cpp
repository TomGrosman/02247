...
namespace {
    ...
    public:
        ...
        bool compareLinesStructural(ProcessedLine compareTo) {
            if (lineOperands.size() == compareTo.lineOperands.size()
                ...
                //errs() << "Lines " << lineNumber << " and " << compareTo.lineNumber << "are not structuraly identical. \n";
                return false;
        }
    ...
    struct CopyPaste5: public ModulePass {
        ...
        void compareMap(std::map<int, ProcessedLine> testMap, CompareType type,
            ...
            switch (type) {
                case operands:
                    while (i != fin - k) {
                        ...
                        if (cpt >= k)
                            ...
                            << " match with respect to their operands\n";
                        i++;
                    }
                    break;
                ...
                case operators:
                    while (i != fin - k) {
                        int j = i + k;
                        //while ((j<fin-k) && testMap.at(i).lineOperators != testMap.at(j).lineOperators  )      j++;
                        while ((j < fin - k)
                        ...
                        int cpt = 1;
                        while ((j + n) < fin && n < (j - i)
                               && (testMap.at(i + n).compareLinesStructural(
                                                                            testMap.at(j + n)))) {
                            cpt++;
                            n++;
                        }
        ...
        virtual bool runOnModule(Module &M) {
                        ...
                        if ((loadInst = dyn_cast < LoadInst > (&I))) {
                            ...
                            if (DILocation *Loc = I.getDebugLoc()) { // Here I is an LLVM instruction
                                ...
                                if (itr == lineMap.end()) { // This is the first operand in this source line
                                    ...
                                    lineMap.emplace(lineNumber,
                                                    ProcessedLine(lineNumber, F.getName(),
                                                                  v));
            ...
            //compareRangesForOperands(5, 2, 12,lineMap, operands);
                        ...
                        && (ELSEBlock->getUniqueSuccessor()
                            ...
                            if (ELSEBlock->size() != THENBlock->size())
                                ...
                                //if (DILocation *Loc = iTHENBlock->getDebugLoc()) { // Here I is an LLVM instruction
                                    //unsigned Line = Loc->getLine();
                                    //errs() << "then block ends at line " << Line
                                    //<< "\n";
                                ...
                                BlockTable.emplace(BlockTableIteratorCounter,
                                                   BlockHolder(thenFirstLine, elseFirstline,
                                                               elseFirstline - thenFirstLine - 1));
                            }
                        }
                }
            }
